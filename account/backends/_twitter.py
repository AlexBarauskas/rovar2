...
class Api(object):
  ...
  def GetFriendsTimeline(self,
    ...
    '''
    if user:
      url = 'http://api.twitter.com/1/statuses/friends_timeline/%s.json' % user
  ...
  def GetUserTimeline(self, user=None, count=None, since=None, since_id=None):
    ...
    if since_id:
      parameters['since_id'] = since_id
    if user:
      url = 'http://api.twitter.com/1/statuses/user_timeline/%s.json' % user
    elif not user and not self._username:
      raise TwitterError("User must be specified if API is not authenticated.")
    else:
  ...
  def GetReplies(self, since=None, since_id=None, page=None): 
    ...
    '''
    url = 'http://api.twitter.com/1/statuses/replies.json'
    if not self._username:
      raise TwitterError("The twitter.Api instance must be authenticated.")
    parameters = {}
    if since:
  ...
  def GetFeatured(self):
    '''Fetch the sequence of twitter.User instances featured on api.twitter.com/1
    ...
    '''
    url = 'http://api.twitter.com/1/statuses/featured.json'
    json = self._FetchUrl(url)
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return [User.NewFromJsonDict(x) for x in data]
  ...
  def GetUserByEmail(self, email):
    ...
    Returns:
      A twitter.User instance representing that user
    '''
    url = 'http://api.twitter.com/1/users/show.json?email=%s' % email
    json = self._FetchUrl(url)
    data = simplejson.loads(json)
    self._CheckForTwitterError(data)
    return User.NewFromJsonDict(data)
